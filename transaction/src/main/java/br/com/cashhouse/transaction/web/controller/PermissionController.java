...
import br.com.cashhouse.transaction.web.dto.GroupPermission;
import br.com.cashhouse.transaction.web.dto.UpdatePermission;
import br.com.cashhouse.util.service.WebSession;

@Controller("WebTransactionPermissionController")
@RequestMapping("/transactions/{id}/permissions")
public class PermissionController {
	
	private static final String INDEX = "redirect:/transactions";
	...
	public String home(@PathVariable("id") Long id, Model model) {
		...
		Acl acl = transactionService.findPermission(id);
		...
		return "transactions/permissions/list";
	}
	...
	public String updateUser(@PathVariable("id") Long id, @ModelAttribute("groupPermission") @Valid GroupPermission groupPermission, Model model) {
		...
		for (UpdatePermission up : updates) {
			transactionService.updatePermission(id, up.getUsername(),up.getPermissions());
		}
