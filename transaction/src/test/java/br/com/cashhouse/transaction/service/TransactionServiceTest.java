...
import br.com.cashhouse.transaction.rest.service.TransactionServiceImpl;
...
public class TransactionServiceTest extends ServiceAuthHelper {
	...
	public void whenUpdateDashboardOwner_thenReturnTransactionObject() throws Exception {
		...
		when(transactionRepository.findByDashboardAndId(any(Dashboard.class), eq(1l)))
				.thenReturn(Optional.of(transaction));
		when(cashierRepository.findByDashboardAndId(any(Dashboard.class), anyLong())).thenReturn(Optional.of(energy));
	...
	public void whenUpdate_thenReturnTransactionObject() throws Exception {
		...
		when(cashierRepository.findByDashboardAndId(any(Dashboard.class), anyLong())).thenReturn(Optional.of(energy));
		when(transactionRepository.save(any(Transaction.class))).thenReturn(transaction);
		when(flatmateRepository.findByDashboardAndId(any(Dashboard.class), anyLong()))
				.thenReturn(Optional.of(flatmateAssign));
	...
	public void whenUpdate_thenThrowFlatmateAssignedEntityNotFoundException() throws Exception {
		...
		when(transactionRepository.findByDashboardAndId(any(Dashboard.class), eq(1l))).thenReturn(Optional.empty());
		when(flatmateRepository.findById(anyLong())).thenReturn(Optional.empty());
	...
	public void whenDelete_thenReturnVoid() throws Exception {
		...
		assertThat(dashboard.getTransactions(), empty());
	...
	public void whenFindBylatmateReferences_thenReturnObjectArray() throws Exception {
		...
		when(transactionRepository.findByDashboardAndFlatmateRef(any(Dashboard.class), any(Flatmate.class),
				any(Flatmate.class))).thenReturn(Collections.emptyList());
	...
	public void whenFindByCashierReferences_thenReturnObjectArray() throws Exception {
		...
		when(transactionRepository.findByDashboardAndCashier(any(Dashboard.class), any(Cashier.class)))
	...
	public void whenActionFinish_thenReturnTransactionObject() throws Exception {
		...
		when(cashierRepository.findById(anyLong())).thenReturn(Optional.of(energy));
		when(transactionRepository.save(any(Transaction.class))).thenReturn(transaction);
